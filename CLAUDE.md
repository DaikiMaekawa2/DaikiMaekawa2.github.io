...
- No aspirational region-wide claims ("across Southeast Asia", 「東南アジア全域へ」) — state only achieved or firmly scheduled facts with dates, e.g. "across Singapore, and from August 2026, in Malaysia"; applies to EN and JA equally
...
- `_data/*.yml` — all editable content: `schedule.yml` (generated — see below), `schedule_overrides.yml`, `timeline.yml`, `menu.yml`, `gallery.yml`, `stats.yml`, `locations.yml`
...
2. `python3 scripts/sync_schedule.py` regenerates `_data/schedule.yml` (public fields only: names, venue types, dates — never financial columns) and warns about scheduled events missing from the footprint map
...
4. Card copy (display name, location, one-liner, all with `_ja` variants) lives in `_data/schedule_overrides.yml`, keyed by `event_id`; `map_skip: true` exempts overseas venues from the map check
